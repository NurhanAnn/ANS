...
def get_access_token():
    ...
    if accounts:
        ...
        if "access_token" in result:
            ...
            st.error(f"Error acquiring token silently: {result.get('error_description')}")
            return None
...
def fetch_email_attachments(access_token):
    """
    Fetches emails and their attachments from the inbox using the Microsoft Graph API.
    """
    url = "https://graph.microsoft.com/v1.0/me/mailFolders/inbox/messages?$expand=attachments"
    ...
    while url:
        ...
        if response.status_code == 200:
            ...
            for email in emails:
                ...
                # Only process emails with 'report' in subject
